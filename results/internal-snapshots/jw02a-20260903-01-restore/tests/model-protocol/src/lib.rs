...
mod tests {
    ...
    #[test]
    fn plain_text_legacy_roundtrip_preserves_wire() {
        for message in [
            ...
            let structured = ModelMessage::try_from(message.clone()).unwrap();
            assert_eq!(ChatMessage::try_from(structured).unwrap(), message);
            assert_eq!(
                wire,
    ...
    #[test]
    fn legacy_tool_text_and_lossy_downcasts_are_rejected() {
        assert_eq!(
            ModelMessage::try_from(ChatMessage {
                role: Role::Tool,
                content: "x".into()
            }),
            Err(ModelProtocolError::LegacyToolAssociationMissing)
        );
        assert_eq!(
            ChatMessage::try_from(result("id")),
            Err(ModelProtocolError::LossyLegacyConversion)
        );
        let message = response(vec![call("id", "lookup")], FinishReason::ToolCalls)
            .into_message()
            .unwrap();
        assert_eq!(
            ChatMessage::try_from(message),
            Err(ModelProtocolError::LossyLegacyConversion)
        );
    }
    ...
    fn existing_adapter_declares_only_implemented_paths_without_probing() {
        let adapter =
            OpenAiLlm::new(OpenAiConfig::new("http://127.0.0.1:1/v1", "not-loaded").unwrap())
                .unwrap();
        assert_eq!(adapter.capabilities(), ModelCapabilities::text_only());
        assert_eq!(
    ...
    impl Llm for CountingLlm {
        // Intentionally omit capabilities to verify source compatibility.
        fn complete<'a>(
            &'a self,
            _: &'a [ChatMessage],
            _: LlmCallOptions,
        ) -> LlmFuture<'a, Result<LlmCompletion, LlmError>> {
            Box::pin(async move {
    ...
    #[tokio::test]
    async fn unknown_preflight_makes_zero_inferences_and_legacy_api_still_works() {
        let calls = Arc::new(AtomicUsize::new(0));
        let provider: Arc<dyn Llm> = Arc::new(CountingLlm(Arc::clone(&calls)));
        assert_eq!(
            native_request(ToolChoice::Auto)
                .preflight(&provider.capabilities(), ModelCallMode::Complete),
            Err(ModelProtocolError::UnknownCapability)
        );
        assert_eq!(calls.load(Ordering::SeqCst), 0);
        assert_eq!(
            provider
                .complete(&[ChatMessage::user("hello")], LlmCallOptions::default())
                .await
                .unwrap()
                .content,
            "legacy answer"
    ...
    #[tokio::test]
    async fn canonical_gateway_preserves_legacy_calls_and_forwards_capabilities() {
        let calls = Arc::new(AtomicUsize::new(0));
