...
| 当前变更 | 已实现统一模型协议及可选单步动作组件，原生/JSON 动作、受控工具调用、Task/Step 关联已贯通；同步指南和独立测试。用户已授权将当前开发资产提交并推送到 dev |

> 2026-09-03 开发分支策略修订：dev 纳入除所有层级 target 目录外的开发资产，
> 包括测试、脚本、锁文件、内部文档与验证记录；Cargo 发布包边界保持独立。
> 下文保留 M0 与各实施阶段的历史事实，其中“被忽略”“未提交/推送”“私有”不再描述
> 当前 dev 的跟踪范围或访问权限。dev 不是私有存储；本次不更新 main 或执行正式发布。
...
| workspace | M0 初始为 15 个 crate；JW-02-c 新增可选 jingwei-action 后为 16 个；edition 2024 |
...
docs/guide/src 已包含概览、模型生成、单步动作和开发环境说明，5 个 Rust 教学片段由私有工程提取为 doctest 并通过。尚未安装 mdBook 或创建指南站构建配置，不宣称完整指南已完成。

## 7. 接下来可以按顺序执行的工作

1. JW-02 的单步动作解析/执行闭环已接入；接下来按 JW-04 推进任务预算和模型 admission，JW-03 的真实后端双模式仍须准备模型后验收，不把私有假模型/回环测试视为真实模型评测。
...
最新单步动作阶段：docs/internal/implementation/JW-02-c.md；results/baseline/20260903-230901-998 的 9/9 检查和 91 项测试通过，16/16 包清单通过。预算、模型 admission、完整参考 Agent、真实模型与恢复仍在后续阶段。

本节保留 JW-02-a 当时的代码位置、兼容决策与验证证据；当前接口已由 ADR-0002 / JW-02-b 直接替换，不再保留旧文本 API。

JW-02-b 阶段验证：results/baseline/20260903-222711-594，8/8 检查通过，14 个保留内嵌用例、52 个私有用例和 4 个指南 doctest 共 70 个通过；15/15 包清单检查通过。详细改动、历史用例替换与未完成门槛见 docs/internal/implementation/JW-02-b.md。后续单步动作阶段以 JW-02-c 记录为准。
