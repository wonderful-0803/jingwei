...
workspace 目前由 16 个 crate 组成，包版本暂为 0.1.0；这是开发中的 v0.1 功能里程碑，不表示这些新增功能已经发布到 crates.io。jingwei-action 是独立可选组件，facade 的默认 feature 不启用它。
...
`& .\scripts\check-baseline.ps1 -Jobs 4`，重跑 9 项离线基线检查。
脚本依赖本机 Rust 工具链和已缓存依赖，日志写入 results/baseline。
