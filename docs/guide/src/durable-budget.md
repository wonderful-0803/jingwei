...
宿主负责可信存储、Task/Session/Agent 映射和 Session 的单一写者。默认 JSONL Session adapter 已提供跨进程排他写者锁，贯穿 provider 生命周期；交接需停止、排空并释放旧句柄。自定义 provider 仍需提供同等保护。部署和恢复步骤见[预算候选恢复](budget-recovery.md)。
...
没有恢复审计的新导出快照为数字版本 3，带恢复审计的快照为版本 4；保留 V2 的持久占用语义和 V3 的宿主审计增额。V1 普通镜像、V2 占用仍可读；旧版本不能携带 V3 审计。Session TaskRunReport 与文件记录外层仍各自使用版本 1。旧实现应明确拒绝未知版本，不回退解释。

## 尚未交付

宿主安全边界上的增额已提供，见[审计增额](budget-grants.md)。保留原始最终候选时可进行[审计恢复](budget-recovery.md)，其他中断冻结及完整故障核验仍在后续；本批不提供未完成工具自动重试或完整 AskUser 业务状态恢复。完整 Task 状态与工具幂等性在 JW-07，仍是 v0.1 必须验收的内容。
