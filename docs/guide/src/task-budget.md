...
后续共享预算接入需要在 Session/模型排队之前开始 Task 活动计时。canonical 模型调度已有自己的单次 admission 截止时间，但尚未与此账本共享时间或资源消耗，不能把单次超时当作 Task 累计时间上限。

## 当前边界与下一步

预算报告虽可序列化，但不是持久快照，反序列化不能创建可执行账本。当前没有跨进程恢复、版本化预算日志、CAS 更新或审计增额；跨 run 累计只限于同一内存账本。v0.1 后续恢复必须保留累计消耗，不能通过重启重置预算。

[有限模型调度](model-scheduling.md)已经提供执行槽、等待队列、总在途容量与单次超时控制。后续继续接入 Agent/Model/Tool 的共享 Task 预算与停止报告，再实现预算持久化和恢复。无显式 Task 的现有 Echo/自定义 Agent 如何绑定默认有限作用域，以及单调用预算如何进一步收紧 Task 上限，都属于共享预算集成阶段的决策。现有模型调用具备调度限制，但尚未自动消费此账本，不能据此声称已经受到 Task 累计预算保护。
