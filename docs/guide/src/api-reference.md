...
| 工具副作用与显式重试 | `jingwei::tool::{ToolEffect, prepare_tool_retry}` | [工具重试](tool-retries.md) |
...
默认 façade 不启用可选 action/context/reference-agent/task-state 组件，也不包含文件检查点适配器。只引入应用实际需要的组件，不把指南目录或验证工程加入 Cargo 依赖。
