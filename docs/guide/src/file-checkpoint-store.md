...
`load(&identity)` 返回经过验证并同步确认的最新镜像。它需要写权限（用于确认此前完整但未获回执的记录），但不会授予执行权。先 load 再 restore 并不是排他的恢复操作：两个宿主可能同时得到相同的预算镜像。持久运行协调器尚未接入，宿主不能只靠本章 API 就并发恢复任务。
...
文件存储是 d2 的第一批。运行前持久占用、排他恢复、Session 日志/检查点一致性与宿主增额审计仍在后续 d2 工作中；不把本章完成视为 AT-F4-04 或完整任务恢复已验收。
