...
已提供底层预算快照、旧账本封存和未决状态冻结。显式持久模式通过 BudgetExecutionLease 取得唯一运行占用，canonical runtime 核验日志并提交最终检查点。宿主可在安全边界审计增额；持有原始最终候选时可审计恢复，其他中断冻结及完整 Task 状态恢复尚未交付。
...
14. 后续按实现进度补充完整装配入门、上下文、参考 Agent 及完整恢复教程。
